...
class AutoUpdater {
    constructor(botInstance = null) {
        ...
        console.log(`\x1b[36m✅ Auto-Updater: Will check every ${this.checkInterval} seconds\x1b[0m`);
    ...
    startMonitoring() {
        ...
        // DO NOT check immediately on startup
        // Wait for the full interval before first check
        const now = new Date();
        const nextCheck = new Date(now.getTime() + (this.checkInterval * 1000));
        console.log(`\x1b[36m⏰ First check in ${this.checkInterval} seconds (at ${nextCheck.toLocaleTimeString()})\x1b[0m`);
    ...
    async performCheck() {
        if (this.isUpdating) {
            // If updating, reschedule check for later
        ...
        try {
            ...
            } else if (latestCommit !== this.lastCommit) {
            ...
            console.log('\x1b[33m⚠️ Check failed\x1b[0m');
        }
